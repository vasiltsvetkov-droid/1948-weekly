import { KNOWLEDGE, interpolate } from '../knowledge'

/**
 * Generate recommendations per player based on computed metrics.
 * Rules read thresholds from KNOWLEDGE modules and templates from MD files.
 * Each recommendation now includes postulates[] and detailedAnalysis.
 *
 * @param {Object} metrics - Output from computeMetrics()
 * @param {number} personalMaxSpeed - Player's recorded max speed (km/h)
 * @returns {{ type: string, title: string, text: string, ref: string, topic: string, postulates: string[], detailedAnalysis: string }[]}
 */
export function generateRecommendations(metrics, personalMaxSpeed) {
  ...
  // Helper to get postulates from a knowledge module
  const getPostulates = (topic) => KNOWLEDGE[topic]?.postulates || []
  // Helper to build detailed analysis from templates
  const getDetail = (topic, zone, data) => {
    const tpl = KNOWLEDGE[topic]?.templates?.[zone] || KNOWLEDGE[topic]?.templates?.default || ''
    return tpl ? interpolate(tpl, data) : ''
  }

  const metricData = {
    acwr: acwr != null ? acwr.toFixed(2) : 'N/A',
    acwrNrg: acwrNrg != null ? acwrNrg.toFixed(2) : 'N/A',
    fatigueIndex: fatigueIndex != null ? fatigueIndex.toFixed(2) : 'N/A',
    monotony: monotony != null && isFinite(monotony) ? monotony.toFixed(2) : 'N/A',
    loadHsr: loadHsr != null ? loadHsr.toFixed(0) : 'N/A',
    loadSprint: loadSprint != null ? loadSprint.toFixed(0) : 'N/A',
    loadHmld: loadHmld != null ? loadHmld.toFixed(0) : 'N/A',
    loadNrg: loadNrg != null ? loadNrg.toFixed(0) : 'N/A',
    loadAcc: loadAcc != null ? loadAcc.toFixed(0) : 'N/A',
    loadDec: loadDec != null ? loadDec.toFixed(0) : 'N/A',
    injuryRisk: (injuryRisk / 10).toFixed(1),
    rtt: rtt != null ? (rtt / 10).toFixed(1) : 'N/A',
    rs: rs != null ? (rs / 10).toFixed(1) : 'N/A',
    topSpeed: metrics.top_speed != null ? metrics.top_speed.toFixed(1) : 'N/A',
    personalMaxSpeed: personalMaxSpeed != null ? personalMaxSpeed.toFixed(1) : 'N/A',
    speedPct: personalMaxSpeed && metrics.top_speed ? ((metrics.top_speed / personalMaxSpeed) * 100).toFixed(0) : 'N/A',
    acwrMechanical: metrics.acwr_mechanical != null ? metrics.acwr_mechanical.toFixed(2) : 'N/A',
    mechPctOver: metrics.acwr_mechanical != null ? Math.round((metrics.acwr_mechanical - 1) * 100) : 0,
    hsrExcess: loadHsr != null ? (loadHsr - 100).toFixed(0) : '0',
    nrgExcess: loadNrg != null ? (loadNrg - 100).toFixed(0) : '0',
  }
  ...
  if (acwr !== null) {
    if (acwr > 1.5) {
      push({
        ...
        postulates: getPostulates('load-management'),
        detailedAnalysis: getDetail('load-management', 'danger', metricData),
      })
    } else if (acwr >= 1.3) {
      push({
        type: 'caution',
        ...
        topic: 'load-management',
        postulates: getPostulates('load-management'),
        detailedAnalysis: getDetail('load-management', 'caution', metricData),
      })
    } else if (acwr < 0.8) {
      push({
        type: 'caution',
        ...
        topic: 'load-management',
        postulates: getPostulates('load-management'),
        detailedAnalysis: getDetail('load-management', 'under_prepared', metricData),
      })
    } else {
      push({
        type: 'positive',
        ...
        topic: 'load-management',
        postulates: getPostulates('load-management'),
        detailedAnalysis: getDetail('load-management', 'optimal', metricData),
      })
    }
  }
  ...
  if (fatigueIndex !== null) {
    if (fatigueIndex > 5.0) {
      push({
        ...
        postulates: getPostulates('fatigue-monitoring'),
        detailedAnalysis: getDetail('fatigue-monitoring', 'high_fatigue', metricData),
      })
      ...
      push({
        ...
        topic: 'fatigue-monitoring',
        postulates: getPostulates('fatigue-monitoring'),
        detailedAnalysis: getDetail('fatigue-monitoring', 'mild_fatigue', metricData),
      })
      ...
      push({
        ...
        topic: 'fatigue-monitoring',
        postulates: getPostulates('fatigue-monitoring'),
        detailedAnalysis: getDetail('fatigue-monitoring', 'low_fatigue', metricData),
      })
    }
  }
    ...
    if (acwrMech !== null) {
      if (acwrMech > 1.4) {
        ...
        push({
          ...
          topic: 'mechanical-load',
          postulates: getPostulates('mechanical-load'),
          detailedAnalysis: getDetail('mechanical-load', 'overload', metricData),
        })
        ...
        push({
          ...
          topic: 'mechanical-load',
          postulates: getPostulates('mechanical-load'),
          detailedAnalysis: getDetail('mechanical-load', 'rising', metricData),
        })
      }
    }
  }
  ...
  if (monotony !== null && isFinite(monotony)) {
    if (monotony > 2.0) {
      push({
        ...
        topic: 'training-monotony',
        postulates: getPostulates('training-monotony'),
        detailedAnalysis: getDetail('training-monotony', 'high', metricData),
      })
      ...
      push({
        ...
        topic: 'training-monotony',
        postulates: getPostulates('training-monotony'),
        detailedAnalysis: getDetail('training-monotony', 'moderate', metricData),
      })
    }
  }
  ...
  if (loadHsr !== null) {
    if (loadHsr < 70) {
      push({
        ...
        topic: 'speed-exposure',
        postulates: getPostulates('speed-exposure'),
        detailedAnalysis: getDetail('speed-exposure', 'hsr_deficit', metricData),
      })
    } else if (loadHsr > 150) {
      push({
        type: 'caution',
        ...
        topic: 'speed-exposure',
        postulates: getPostulates('speed-exposure'),
        detailedAnalysis: getDetail('speed-exposure', 'hsr_excess', metricData),
      })
    }
  }
  ...
  if (loadSprint !== null && loadSprint < 100) {
    push({
      ...
      topic: 'speed-exposure',
      postulates: getPostulates('speed-exposure'),
      detailedAnalysis: getDetail('speed-exposure', 'sprint_deficit', metricData),
    })
  }
  ...
  if (personalMaxSpeed && metrics.top_speed) {
    ...
    if (speedPct < 0.90) {
      push({
        ...
        topic: 'speed-exposure',
        postulates: getPostulates('speed-exposure'),
        detailedAnalysis: getDetail('speed-exposure', 'vmax_deficit', metricData),
      })
    }
  }
  ...
  if (loadHmld !== null && loadHmld < 70) {
    push({
      ...
      topic: 'mechanical-load',
      postulates: getPostulates('mechanical-load'),
      detailedAnalysis: getDetail('mechanical-load', 'hmld_deficit', metricData),
    })
  }
  ...
  if ((loadAcc !== null && loadAcc < 70) || (loadDec !== null && loadDec < 70)) {
    push({
      ...
      topic: 'mechanical-load',
      postulates: getPostulates('mechanical-load'),
      detailedAnalysis: getDetail('mechanical-load', 'accdec_deficit', metricData),
    })
  }
  ...
  if (loadNrg !== null && loadNrg > 130) {
    push({
      ...
      topic: 'match-demands',
      postulates: getPostulates('match-demands'),
      detailedAnalysis: getDetail('match-demands', 'nrg_overload', metricData),
    })
  }
  ...
  if (rtt !== null && rtt < 40) {
    push({
      ...
      topic: 'periodization',
      postulates: getPostulates('periodization'),
      detailedAnalysis: getDetail('periodization', 'low_rtt', metricData),
    })
  }
  ...
  if (injuryRisk !== null && injuryRisk < 40) {
    ...
    push({
      ...
      topic: 'injury-risk',
      postulates: getPostulates('injury-risk'),
      detailedAnalysis: getDetail('injury-risk', 'elevated', metricData),
    })
  }

  // Rule 13: Recovery Status Low (NEW — driven by recovery-protocols knowledge)
  if (rs !== null && rs < 40) {
    push({
      type: 'alert',
      title: 'Poor Recovery Status — Intervention Required',
      text: `Recovery Status of ${(rs / 10).toFixed(1)}/10 indicates inadequate recovery between sessions. The internal load (cardiovascular cost) is disproportionately high relative to external output. Prioritize MD+1 active recovery (HR <65% HRmax, total distance <2000m) and rest on MD+2. Sleep extension to 9–10h improves reaction time and sprint speed (Mah et al. 2011). Contrast therapy and nutrition (CHO 1–1.5 g/kg within 30 min post-session) are critical interventions.`,
      ref: 'Dupont et al. (2010); Mah et al. (2011); Saw et al. (2016); §7.3, §8.3',
      topic: 'recovery-protocols',
      postulates: getPostulates('recovery-protocols'),
      detailedAnalysis: getDetail('recovery-protocols', 'poor_recovery', metricData),
    })
  }
